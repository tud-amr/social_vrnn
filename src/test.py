...
if test_args.record:
	...
	if ("real_world" in test_args.scenario) and not test_args.unit_testing:
		...
		# Generate video
		try: recorder.plot_on_image(input_list, grid_list, all_predictions, y_ground_truth_list, other_agents_list, trajectories, test_args)
		except: pass
		# Generate zoomed-in video
		try: Plotter.generate_zoom_plot(recorder.args, *plotargs)
		except: pass
	else:
